...
anchor = '        {historicalData && !dataLoading && quantStats && (\n          <div className="flex-1 overflow-y-auto min-h-[500px] flex flex-col gap-8 pb-12">\n'
anchor_idx = text.find(anchor)
start_idx = anchor_idx + len(anchor)

end_anchor = "            {/* PROBABILISTIC SCENARIOS */}"
...
sect = sect.replace('<div className="flex flex-col mt-8 border border-[#141414] bg-white">', '<div className="flex flex-col border border-[#141414] bg-white h-full">', 1)
...
new_text = text[:start_idx] + new_dash + text[end_idx:]
