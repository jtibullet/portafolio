...
async function main() {
    ...
    const expDateStr = opts.expirationDates.length > 1 ? opts.expirationDates[1] : opts.expirationDates[0];
    const optsChain = await yahooFinance.options(ticker, { date: expDateStr });
    const calls = optsChain.options[0].calls;
    
    const T = (new Date(expDateStr).getTime() - Date.now()) / (1000 * 60 * 60 * 24 * 365);
    ...
    const coeffs = polyFit(logMoneyness, ivs, 3);
    ...
    let densities: {k: number, density: number}[] = [];
    for (let K = minK + dK; K < maxK - dK; K += dK) {
        ...
        const iv = polyEval(coeffs, lm);
        const ivMinus = polyEval(coeffs, lmMinus);
        const ivPlus = polyEval(coeffs, lmPlus);
        
        const c = blackScholesCall(S, K, T, r, Math.max(0.01, iv));
        const cMinus = blackScholesCall(S, kMinus, T, r, Math.max(0.01, ivMinus));
        const cPlus = blackScholesCall(S, kPlus, T, r, Math.max(0.01, ivPlus));
        
        const secondDeriv = (cPlus - 2 * c + cMinus) / (dK * dK);
        let density = Math.exp(r * T) * secondDeriv;
        if (density < 0) density = 0;
        
        densities.push({ k: K, density });
    }
    ...
    console.log(`Prob > 20%: ${(prob20 * 100).toFixed(2)}%`);
    console.log(`Densities size: ${densities.length}`);
}
