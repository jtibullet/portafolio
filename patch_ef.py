...
old_chart_code = """                  <div className="flex flex-col h-full bg-white border border-[#141414]">
                    <div className="flex justify-between items-end mb-2">
                      <h3 className="text-xs font-mono uppercase bg-[#141414] text-[#E4E3E0] px-2 py-0.5">Efficient Frontier</h3>
                    </div>
                    <div className="flex-1 bg-[#111111] border border-[#141414] p-4 min-h-[300px]">
                      {(() => {
                        const simulated = quantStats.optimization.simulated || [];
                        const maxSharpe = Math.max(...simulated.map((p: any) => p.sharpeRatio), quantStats.optimization.optimal.sharpeRatio, 0);
                        const minSharpe = Math.min(...simulated.map((p: any) => p.sharpeRatio), 0);
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                              ...
                              <XAxis allowDuplicatedCategory={false} 
                                ...
                                label={{ value: 'Annualized Volatility (Risk)', position: 'insideBottom', offset: -10, fontSize: 10, fontFamily: 'monospace', fill: '#888' }}
                                stroke="#444"
                              ...
                              <YAxis 
                                ...
                                domain={['auto', 'auto']}
                                tickFormatter={(val) => `${(val * 100).toFixed(1)}%`}
                                label={{ value: 'Expected Return', angle: -90, position: 'insideLeft', fontSize: 10, fontFamily: 'monospace', fill: '#888' }}
                                stroke="#444"
                                tick={{ fill: '#888', fontSize: 10 }}
                              />
                              <Tooltip 
                                ...
                                formatter={(value: number, name: string, props: any) => {
                                  ...
                                  if (name === 'Optimal Portfolio' && props.payload.weights) {
                                      ...
                                      const wStr = quantStats.matrixSymbols.map((s: string, i: number) => `${s}: ${(w[i] * 100).toFixed(1)}%`).join(', ');
                              ...
                              >
                                {simulated.map((entry: any, index: number) => (
                                  <Cell key={`cell-${index}`} fill={interpolatePlasma((entry.sharpeRatio - minSharpe) / sharpeRange)} />
                                ))}
                              </Scatter>
                              <Line 
                                data={quantStats.optimization.frontier} 
                                type="basis" 
                                dataKey="expectedReturn" 
                                stroke="#fff" 
                                strokeWidth={2} 
                              ...
                              <Scatter 
                                name="Assets" 
                                data={quantStats.assetStats.map((s: any) => ({
